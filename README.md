## Shubham Jain

**Software Engineer** · Building Scalable SaaS Products

[Portfolio](https://shubhm.in/) · [LinkedIn](https://www.linkedin.com/in/shubhmjain/) · [Twitter](https://x.com/shubhmj30)

---

Building production-grade web applications across the frontend and backend,
with a focus on performance, maintainability, and developer experience.

*Clarity over cleverness. I write code for the next developer, not for my ego.*

### Now
🔧 Building SaaS products across frontend and backend  
📖 Deep-diving into JavaScript internals & system design  
🌱 Exploring meaningful open-source contributions  

### Stack
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-20232A?style=flat&logo=typescript&logoColor=3178C6)
![Node.js](https://img.shields.io/badge/Node.js-20232A?style=flat&logo=node.js&logoColor=339933)
![Express](https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-20232A?style=flat&logo=mongodb&logoColor=47A248)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-20232A?style=flat&logo=tailwindcss&logoColor=38BDF8)

---

*Always learning. Always shipping.*
